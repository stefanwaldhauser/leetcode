...
def solve_exact(m,n,t, mem):
    ...
    if t == 0: #  # If exactly 0 time is left, it means we've used up the time perfectly without needing a burger
        mem[t] = 0
        return mem[t] # No time means no burger
    ...
    m_choice_result = solve_exact(m,n, t-m, mem)
    ...
    n_choice_result = solve_exact(m,n, t-n, mem)
    ...
    if best_outcome == -1: # smaller problem already not solvable with m or n so larger definitely also not solvable
        mem[t] = -1
        return mem[t]
    mem[t] = best_outcome + 1 # one burger more
    return mem[t]
